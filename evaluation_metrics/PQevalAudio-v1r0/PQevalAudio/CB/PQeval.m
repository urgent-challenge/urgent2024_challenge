...
% Modulation patterns
%[M, ERavg, Fmem.Env] = PQModPatt2 (Es, Fmem.Env);
...
for (i = 1:2)
    for (m = 0:Nc-1)
        Ee = Es(i,m+1)^e;
        Fmem_env.DE(i,m+1) = a(m+1) * Fmem_env.DE(i,m+1) ...
                       + b(m+1) * Fss * abs (Ee - Fmem_env.Ese(i,m+1));
        Fmem_env.Eavg(i,m+1) = a(m+1) * Fmem_env.Eavg(i,m+1) + b(m+1) * Ee;
        Fmem_env.Ese(i,m+1) = Ee;

        M(i,m+1) = Fmem_env.DE(i,m+1) / (1 + Fmem_env.Eavg(i,m+1)/0.3);
    end
end

ERavg = Fmem_env.Eavg(1,:);
Fmem.Env = Fmem_env;
...
%--------------------
function [M, ERavg, Fmem] = PQmodPatt2 (Es, Fmem)
