...
unzip "${output_dir}/DS_10283_3443.zip" -d "${output_dir}"
unzip "${output_dir}/VCTK-Corpus-0.92.zip" -d "${output_dir}/VCTK-Corpus"
...
done | awk -F '/' '{print($NF" 48000 "$0)}' | sed -e 's/\.wav / /g' | sort -u > vctk_train.scp
