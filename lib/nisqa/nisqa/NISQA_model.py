...
class nisqaModel(object):
    ...
    def _loadDatasets(self):
        ...
        elif self.args['mode']=='predict_scp':
            self._loadDatasetsSCP()
        elif self.args['mode']=='main':
            self._loadDatasetsCSV()
        else:
            raise NotImplementedError('mode not available')                        

    def _loadDatasetsSCP(self):

        files = {}
        with open(self.args['input_scp'], "r") as f:
            for line in f:
                uid, audio_path = line.strip().split()
                files[uid] = audio_path
        shutil.rmtree('./tmp/audios',)
        os.makedirs('./tmp/audios', exist_ok=True)
        for uid in files:
            os.symlink(files[uid], f'./tmp/audios/{os.path.basename(files[uid])}.wav')
        self.args['data_dir'] = './tmp/audios'
